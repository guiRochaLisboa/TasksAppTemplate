...
class TaskViewHolder(itemView: View, val listener: TaskListener) :
    ...
    fun bindData(task: TaskModel) {
        ...
        this.mTextPriority.text = mPriorityRepository.getDescirption(task.priorityId).toString()


        //Nosso dueDate está vindo com yyyy-MM-dd temos que tranformar em dd-MM-yyyy com o nosso mDateFormat
        val date = SimpleDateFormat("yyyy/mm/dd").parse(task.dueDate)
        this.mTextDueDate.text = mDateFormat.format(date)


        //Verificação para saber se a nossa Task está completa ou não, trocando a imagem e o texto caso esteja.
        if(task.complete){
